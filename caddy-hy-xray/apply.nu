...
let config = (open config.yaml | insert CERT_DIR_NAME ($in.CURRENT_DOMAIN | str replace '\*.' 'wildcard_.') | transpose key value)

ls *.template | get name | 
each { |template_file_name| 
    ...
    let final_file = ($config | reduce --fold $template_file { |it, acc|
